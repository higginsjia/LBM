...
void calVelovityField(double U_[Nx][Ny])
{
	int i, j;
	double u_, v_, rho_;
	for (i = 1; i <Nx - 1; i++)
...
void calDif()
{
	...
	for (int j = 0; j < Ny;++j)
	{
		u_error += std::fabs(Ucur[i][j] - Unex[i][j]);
		u_sum += Ucur[i][j];
	}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		calVelovityField(Ucur);
		evolute();
		calVelovityField(Unex);
...
#pragma omp parallel for
	...
	for (j = 0; j < Ny; j++)
	{
		...
		for (k = 0; k <= 8; k++){
			rho[i][j] += f_old[i][j][k];
			u[i][j] += cx[k] * f_old[i][j][k];
			v[i][j] += cy[k] * f_old[i][j][k];
		}
	...
	for (j = 0; j < Ny; j++)
	{
		...
		for (k = 0; k <= 8; k++)
			f_old[i][j][k] = (1.0 - omega_les)*f_old[i][j][k]
