...
void evolute()
{
	...
	for (j = 1; j <Ny - 1; j++)
	{
		if (flag[i][j] == fluid)
		{
			...
			{
				int inv_ = inv[k];
				if (flag[i + (int)cx[inv_]][j + (int)cy[inv_]] == boundary)
					f_new[i][j][k] = f_old[i][j][inv[k]];
				else
	...
	for (j = 1; j < Ny - 1; j++)
	{
		if (flag[i][j] != boundary)
		{
			...
			//using feq=w*(rho+...+...)-->u=ci*fi/rho=ci*fi/1=ci*fi-->ref to feq()
			//inlet velocity Uin
			if (flag[i][j] == inlet)
			{
				rho[i][j] = rho[i][j - 1];//////////////////////set density according to near cell
				u[i][j] = 0.0;
				v[i][j] = -Uin;
			}
			if (flag[i][j] == outlet)
